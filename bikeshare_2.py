### Python code to analyze bikeshare information ###
...
def raw_data_print(df, marker = 0): #setting marker to the 0th position
    ...
    while True:
        for i in range(marker, len(df.index)):
            ...
            choice = str(input("\nDo you want to keep printing raw data? Say yes for printing. Say anything else to proceed to statistics")).lower()
            if choice == 'yes':
                continue
            else:
                break
...
def main():
    while True:
        ...
        raw_data_choice = str(input("\nDo you want to see the raw data before printing statistics? Say yes to print raw data. Say anything else to proceed with calculating statistics")).lower()
